...
@strax.takes_config(
    ...
    *HITFINDER_OPTIONS)
class PulseProcessingFastSampling(strax.Plugin):
    """
    Split raw_records_v1730 into:
     - records_v1730
     - (aqmon_records?)
     - pulse_counts_v1730
    ...
    pulse_counts_v1730 holds some average information for the individual PMT
    ...
    provides = ('records_v1730','pulse_counts_v1730')
    depends_on = ('raw_records_v1730')
    data_kind = {k: k for k in provides}
    parallel = 'process'
    rechunk_on_save = immutabledict(
        records_v1730=False,
        pulse_counts_v1730=True)
    compressor = 'lz4'
    ...
    def infer_dtype(self):
        ...
        dtype = dict() 
        dtype['records_v1730'] = strax.record_dtype(self.record_length)                
        dtype['pulse_counts_v1730'] = pulse_count_dtype(self.config['n_tpc_pmts'])
        
        return dtype

    def compute(self, raw_records_v1730, start, end):
        if self.config['check_raw_record_overlaps']:
            check_overlaps(raw_records_v1730, n_channels=3000) 

        # Throw away any non-TPC records; this should only happen for XENON1T
        # converted data
        raw_records_v1730 = raw_records_v1730[
            raw_records_v1730['channel'] < self.config['n_tpc_pmts']]

        # Convert everything to the records data type -- adds extra fields.
        r = strax.raw_to_records(raw_records_v1730)
        del raw_records_v1730
...
#         # r = r[np.average(r['data']) > 0]
        ...
        return dict(records_v1730=r,
                    pulse_counts_v1730=pulse_counts)
