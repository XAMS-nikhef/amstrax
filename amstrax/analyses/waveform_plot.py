# From https://github.com/XENONnT/amstrax/blob/master/amstrax/analyses/waveform_plot.py
...
def plot_waveform(
    ...
    **kwargs,
    ...
    if not deep:
        ...
        plt.sca(axes[0])
        context.plot_peaks(
            **kwargs, show_largest=show_largest, single_figure=False, xaxis=False
        )
    ...
    **kwargs,
    ...
    if cbar_loc is not None:
        ...
        cax = inset_locator.inset_axes(
            bbox, width="90%", height="20%", loc="upper center"
        )
...
def seconds_range_xaxis(seconds_range, t0=None):
    ...
    if print_ms and t0 is None:
        ...
        if print_us:
            labels = [
                l + r" $\bf{" + f"{us[i]:03}" + "}$" for i, l in enumerate(labels)
            ]
            if print_ns:
