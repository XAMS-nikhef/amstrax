...
ma_doc = (
    """
This is an amstrax mini-analysis.
...
def mini_analysis(
    ...
    def decorator(f):
        ...
        def wrapped_f(context: strax.Context, run_id: str, **kwargs):
            ...
            if warn_beyond_sec is not None and not kwargs.get("ignore_time_warning"):
                ...
                if sec_requested > warn_beyond_sec:
                    tr_str = (
                        "the entire run" if tr is None else f"{sec_requested} seconds"
                    )
                    raise ValueError(
            ...
            if len(requires):
                ...
                for dkind, dtypes in deps_by_kind.items():
                    if dkind in kwargs:
                        print(f"Already have {dkind} data, just apply cuts")
                ...
                # If the user did not give time kwargs, but the function expects
                ...
                if len(x) and kwargs.get("time_range") is None:
                    ...
                    try:
                        kwargs.setdefault(
                            "time_range", (x0["time"], strax.endtime(x).max())
                        )
        ...
        if hasattr(f, "__doc__") and f.__doc__:
            ...
            wrapped_f.__doc__ = (
                "Amstrax mini-analysis for which someone was too lazy"
