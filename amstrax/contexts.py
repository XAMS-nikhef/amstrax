...
def _resolve_run_channel_map(run_doc: dict):
    ...
    run_channel_map = _normalize_channel_map(daq_cfg.get("channel_map"))
    if run_channel_map is not None:
        return run_channel_map
    # Legacy fallback: old runs may have channel_map under xams_bookkeeping.
    xbk = run_doc.get("xams_bookkeeping") if isinstance(run_doc.get("xams_bookkeeping"), dict) else {}
    return _normalize_channel_map(xbk.get("channel_map"))
